...
import SheetManager
from JuneDetectChars import is_normal
...
class Sheet:
        ...
        for col in range(self.dataEndCol):
            ...
            if self.defaultRow == -1:
                ...
                if col == 0:  #第一位缺省值，占位符
                    ...
                    field.default = self.__convertStrTojson_obj(value)
                elif type == 'r':  #引用
                    field.default = value
    ...
    def convert(self,dct):
        if isinstance(dct, dict):
            return {self.convert(key): self.convert(value) for key, value in dct.iteritems()}
        elif isinstance(dct, list):
            return [self.convert(element) for element in dct]
    ...
    #转换字符串为dict
    def __convertStrTojson_obj(self, jstr):
        null = None#json null map to python dict None
        jstr = filter(is_normal,jstr)
        dict = self.convert(eval(jstr))
...
#         print json_obj
            ...
            for col in range(1, self.dataEndCol):
                ...
                if ctype == XL_CELL_EMPTY:  #如果是空的，就填入缺省值
                    ...
                    if fieldType == 'i':
                        ...
                        record[fieldName] = eval(value)
                       # record[fieldName] = self.__convertStrTojson_obj(value)
    ...
    def toPython(self, sheet_output_field=[]):
        ...
        if sheet_output_field == []:
            return self.convert(self.python_obj)
        else:
            new_python_obj = self.convert(self.python_obj.copy())
